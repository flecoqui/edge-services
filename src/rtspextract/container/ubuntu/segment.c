...
static int set_segment_filename(AVFormatContext *s)
{
    ...
    if (seg->use_strftime) {
        ...
        pch = strstr(s->url, timestamp_mask);
        if (pch != NULL) {
            // if rtsp_source_ctx is not NULL, the RTCP clock has been retrieved.
            if(s->rtsp_source_ctx != NULL){
                RTSPState* rt = s->rtsp_source_ctx->priv_data;
                if (rt != NULL) {
                    if (!snprintf(buf, sizeof(buf), s->url, rt->last_ntp_ts)) {
                        av_log(oc, AV_LOG_ERROR, "Could not get segment filename with timestamp\n");
                        return AVERROR(EINVAL);
                    }
                }
                else {
                    av_log(oc, AV_LOG_ERROR, "Could not get RTSPState from AVFormatContext\n");
                    return AVERROR(EINVAL);
                }
            }
            else
            {
                // if rtsp_source_ctx is NULL, the RTCP clock has not been retrieved.
                // the timestamp to 0.0 
                if (!snprintf(buf, sizeof(buf), s->url, 0.0)) {
                    av_log(oc, AV_LOG_ERROR, "Could not get segment filename with timestamp\n");
                    return AVERROR(EINVAL);
                }                
            }
