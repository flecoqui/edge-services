    ...
    if (img->update) {
        ...
        if (pch != NULL) {
            if(s->rtsp_source_ctx != NULL){
                ...
                if (rt != NULL) {
                    if (!snprintf(filename, sizeof(filename), s->url, rt->last_ntp_ts)) {
                        av_log(s, AV_LOG_ERROR, "Could not get frame filename with timestamp\n");
                        return AVERROR(EINVAL);
                    }
                }
                else {
                    av_log(s, AV_LOG_ERROR, "Could not get RTSPState from AVFormatContext\n");
                    return AVERROR(EINVAL);
                }
            }
        }
