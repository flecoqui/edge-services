...
static int update_ntp = 0;
static double update_ntp_timestamp(RTSPState* rtsp_state) {
    ...
    if(update_ntp == 0){
        rtsp_state->last_rtcp_ts = 0;
        rtsp_state->last_ntp_ts = 0;
        rtsp_state->last_basetime = 0;
        update_ntp = 1;
    }
    ...
    if (new_rtcp_ts != rtsp_state->last_rtcp_ts) {
        rtsp_state->last_rtcp_ts = new_rtcp_ts;
        ...
        rtsp_state->last_basetime = seconds + useconds;
    }

    d_ts = rtp_demux_context->timestamp - rtsp_state->last_rtcp_ts;
    rtsp_state->last_ntp_ts = rtsp_state->last_basetime + d_ts / 90000.0;
    av_log(NULL, AV_LOG_DEBUG, "RTSP NTP Timestamp: %018.6f \n", rtsp_state->last_ntp_ts);
    return rtsp_state->last_ntp_ts;
}
