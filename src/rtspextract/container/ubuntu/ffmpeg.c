...
static int transcode_init(void)
{
    ...
    OutputStream *ost;
    InputStream *ist;
    const char* timestamp_mask = "%018.6f";
    const char* rtsp_start = "rtsp://";
    char* pch = NULL;
    char error[1024] = {0};
    ...
    /* if rtsp source and output file segment with absolute timestamp link AVFormatContext */
    for (i = 0; i < nb_output_files; i++) {
        oc = output_files[i]->ctx;
        oc->rtsp_source_ctx = NULL;
        pch = strstr(oc->url, timestamp_mask);
        if (pch != NULL) {
            if ((output_files[i]->ost_index >= 0) && (output_files[i]->ost_index < nb_output_streams)) {
                int index = output_streams[output_files[i]->ost_index]->source_index;
                if ((index >= 0) && (index < nb_input_streams)) {
                    int file_index = input_streams[index]->file_index;
                    if ((file_index >= 0) && (file_index < nb_input_streams)) {
                        ic = input_files[file_index]->ctx;
                        if (strncmp(rtsp_start, ic->url, strlen(rtsp_start)) == 0) {
                            /* find the rtps timestamp scenario */
                            oc->rtsp_source_ctx = ic;
                        }
                    }
                }
            }
